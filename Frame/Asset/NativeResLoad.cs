...
public class NativeResLoad : AssetsBase {
    ...
    public void GetResources(string sceneName, string bundleName, string resName, bool isSingle, ushort backID) {
        if (!ILoadMgr.Instance.IsLoadedAssetBundle(sceneName, bundleName))
        {
            ...
            if (bundleName != null)
            {
                NativeResCallbackNode tmpNode = new NativeResCallbackNode(sceneName, bundleName, resName, isSingle, backID, null, SendToBackMsg);
    ...
    public void SendToBackMsg(NativeResCallbackNode tmpNode) {
        if (tmpNode.isSingle)
        {
            Object tmpObj = ILoadMgr.Instance.GetSingleResource(tmpNode.sceneName, tmpNode.bundleName, tmpNode.resName);
            this.ReleaseBack.Changer(tmpNode.backMsgID, tmpObj);
        }
        else
        {
            Object[] tmpObjs = ILoadMgr.Instance.GetMulResource(tmpNode.sceneName, tmpNode.bundleName, tmpNode.resName);
    ...
    public override void ProcessEvent(MsgBase tmpMsg) {
        ...
        switch (tmpMsg.msgID)
        {
            case (ushort)AssetEvent.HunkRes:
                GetResources(assetRes.sceneName, assetRes.bundleName, assetRes.resName, assetRes.isSingle, assetRes.backMsgID);
                break;
...
public class NativeResCallbackNode {
    ...
    public NativeResCallbackNode(string tmpSceneName, string tmpBundleName, string tmpResName, bool tmpIsSingle, ushort tmpBackMsgID, NativeResCallbackNode tmpNextValue, NativeResCallback tmpCallback)
    {
