...
public partial class BattleLogic : MonoBehaviour {
    ...
    private Dictionary<RoleConfig, float> selectedOutAttackersDic = new Dictionary<RoleConfig, float> (); //被选出的攻击者Dic
    private Dictionary<float, RoleConfig> attackersTimeDistancesDic = new Dictionary<float, RoleConfig> (); //攻击者的时时距离Dic
        ...
        if (testCancel) {
            ...
            while (cancelLoop) {
                for (int i = 0; i < ReadJsonfiles.Instance.allSpeeds.Count; i++) {
                    ...
                    if (firstLoop) {
                        addFl = 0;
                        addFl += float.Parse (ADDUIBattle.instance.speedImages[i].m_RoleInfo.speed.ToString ()) * 0.0001f;
                        savePos_y.Add (addFl);
                    ...
                    if (addFl - roleCon.transform.localPosition.y >= -destination.localPosition.y) {
                        ...
                        roleCon.transform.localPosition = new Vector3 (roleCon.transform.localPosition.x, destination.localPosition.y, roleCon.transform.localPosition.z);
                        ...
                        roleCon.transform.localPosition = new Vector3 (roleCon.transform.localPosition.x, -addFl + roleCon.transform.localPosition.y, roleCon.transform.localPosition.z);
                    }
                ...
                for (int j = 0; j < savePos_y.Count; j++) {
                    if (ADDUIBattle.instance.speedImages[j].transform.localPosition.y >= -destination.localPosition.y) {
                        cancelLoop = false;
                        savePos_y.Clear ();
                        break;
                    } else {
    ...
    void DealAttackImageSortShow () {
        ...
        for (int i = 0; i < ADDUIBattle.instance.speedImages.Count; i++) {
            ...
            if (cancle) {
                if (ADDUIBattle.instance.speedImages[i].GetComponent<RectTransform> ().localPosition.y == ADDUIBattle.instance.speedImages[i + 1].GetComponent<RectTransform> ().localPosition.y) {
                    distance += add;
                    add += 0.0001f;
                }
            }
    ...
    void RoleAttackLogic (RoleConfig roleCon) {
        ...
        for (int j = 0; j < ADDUIBattle.instance.roleSpeedImages.Count; j++) {
            if (roleCon.m_RoleInfo.roleid == ADDUIBattle.instance.roleSpeedImages[j].m_RoleInfo.roleid) {
                isRole = true;
                RoleSpineAniManager.Instance.SendMsg (ButtonMsg.GetInstance.ChangeInfo ((ushort) RoleSpineAniId.action, (ushort) roleconWhole.m_RoleInfo.roleid));
    ...
    void ShowSkillUI (RoleConfig roleCon, bool isRole = true) {
        ...
        Sprite spr;
        if (ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.skillImageDics.ContainsKey (roleCon.m_RoleInfo.skillid1)) {
            ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.skillImageDics.TryGetValue (roleCon.m_RoleInfo.skillid1, out spr);
            skill1_Sprite.sprite = spr;
            skill1_Sprite.name = roleCon.m_RoleInfo.skillid1.ToString ();
        }
        if (ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.skillImageDics.ContainsKey (roleCon.m_RoleInfo.skillid2)) {
            ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.skillImageDics.TryGetValue (roleCon.m_RoleInfo.skillid2, out spr);
            skill2_Sprite.sprite = spr;
            skill2_Sprite.name = roleCon.m_RoleInfo.skillid2.ToString ();
        }
        if (ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.skillImageDics.ContainsKey (roleCon.m_RoleInfo.skillid3)) {
            ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.skillImageDics.TryGetValue (roleCon.m_RoleInfo.skillid3, out spr);
    ...
    IEnumerator WaitFixPos (RoleConfig roleCon) {
        ...
        roleCon.transform.localPosition = new Vector3 (roleCon.transform.localPosition.x, startpoint.localPosition.y, roleCon.transform.localPosition.z);
        roleCon.transform.SetAsFirstSibling ();

    }

    void MonsterAttackLogic (RoleConfig roleCon) {
        roleconWhole = roleCon;
        ...
        for (int j = 0; j < ADDUIBattle.instance.monsterSpeedImages.Count; j++) {
            if (roleCon.m_RoleInfo.roleid == ADDUIBattle.instance.monsterSpeedImages[j].m_RoleInfo.roleid) {
                ...
                if (SkillEffect.Instance.attack_roleIns.m_RoleInfo.isDizziness) {
                    ...
                    MonsterSpineAniManager.Instance.SendMsg (ButtonMsg.GetInstance.ChangeInfo ((ushort) MonsterSpineAniId.action, roleconWhole.iddif));
    ...
    IEnumerator DelaySomeTimeToPlay (float value = 0f) {
        ...
        if (roleconWhole.m_RoleInfo.roleid == 10001)
            RoleSpineAniManager.Instance.SendMsg (ButtonMsg.GetInstance.ChangeInfo ((ushort) RoleSpineAniId.idle, (ushort) roleconWhole.m_RoleInfo.roleid,
    ...
    void DealHeadImage (RoleConfig roleCon) {
        ...
        ColorDebug.Instance.DicDebug<string, Sprite> (ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.roleActionImagesDics, false);

        if (ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.roleActionImagesDics.ContainsKey (roleCon.id.ToString ())) {
            headImage.sprite = ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.roleActionImagesDics[roleCon.id.ToString ()];
        }
