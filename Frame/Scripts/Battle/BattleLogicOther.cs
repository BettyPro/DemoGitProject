...
public partial class BattleLogic : MonoBehaviour {
    ...
    void BloodsFllow () {
        ...
        if (beginFllow) {
            if (SkillEffect.Instance.attack_roleIns.iddif == 0) {
                currenBlood = CreatSkeleton.Instance.BloodsObjDic[SkillEffect.Instance.attack_roleIns.id].GetComponent<RectTransform> ();
                cubeV2Pos = RectTransformUtility.WorldToScreenPoint (Camera.main, SkillEffect.Instance.attack_roleIns.transform.position);
                currenBlood.position = cubeV2Pos + new Vector2 (50, 200);
            } else {
                currenBlood = CreatSkeleton.Instance.BloodsObjDic[SkillEffect.Instance.attack_roleIns.iddif].GetComponent<RectTransform> ();
                cubeV2Pos = RectTransformUtility.WorldToScreenPoint (Camera.main, SkillEffect.Instance.attack_roleIns.transform.position);
    ...
    void AllDamage () {
        ...
        if (saveData) {
            ...
            for (int i = 0; i < CreatSkeleton.Instance.monsterSkeletonsMonsterBattleAni.Count; i++) {
                saveMonsterSkeletonsMonsterBattleAni.Add (CreatSkeleton.Instance.monsterSkeletonsMonsterBattleAni[i]);
            }
        ...
        if (SkillEffect.Instance.attack_roleIns.tag == "Hero" && isRole) {
            for (int i = 0; i < saveMonsterSkeletonsMonsterBattleAni.Count; i++) {
                SkillEffect.Instance.target_roleIns = saveMonsterSkeletonsMonsterBattleAni[i].GetComponent<RoleConfig> ();
    ...
    void SingleRoleAddBloodToEnd (RoleConfig target_roleIns, bool isSingle = true) {
        ...
        RoleSpineAniManager.Instance.SendMsg (ButtonMsg.GetInstance.ChangeInfo ((ushort) RoleSpineAniId.idle, roleconWhole.id, "normal"));
    ...
    RoleConfig MonsterSelectTarget (RoleConfig target_roleIns) {
        ...
        int key = 0;
        for (int i = 0; i < CreatSkeleton.Instance.monsterSkeletonsMonsterBattleAni.Count; i++) {
            monsterSelectLife.Add (CreatSkeleton.Instance.monsterSkeletonsMonsterBattleAni[i].GetComponent<RoleConfig> ().m_RoleInfo.life);
            monsterSelectLifeDic.Add (i, CreatSkeleton.Instance.monsterSkeletonsMonsterBattleAni[i].GetComponent<RoleConfig> ().m_RoleInfo.life);
        }
    ...
    void BloodChangedShow (RoleConfig target_roleIns, float damage, bool isAll = false) {
        ...
        if (CreatSkeleton.Instance.BloodsDic.ContainsKey (id)) {
            ...
            if (isAdd) {
                if (isAll) {
                    if (SkillEffect.Instance.save_target_life_old.Count != 0) {
                        SkillEffect.Instance.target_life_old = SkillEffect.Instance.save_target_life_old[recordWhickCount];

                        if (recordWhickCount + 1 == SkillEffect.Instance.save_target_life_old.Count)
                            SkillEffect.Instance.save_target_life_old.Clear ();

                        if (SkillEffect.Instance.target_life_old == ReadJsonfiles.Instance.roleDics[target_roleIns.id].life) return;
                        if (SkillEffect.Instance.target_life_old + addBloodValue <= ReadJsonfiles.Instance.roleDics[target_roleIns.id].life) {
                            // changedBloodText.text = "<color=green>+" + addBloodValue.ToString ("f0") + "</color>";
                            ValueDealToImage (managerValue, addBloodValue, isAdd);

                        } else {
                            addRealBloodValue = ReadJsonfiles.Instance.roleDics[target_roleIns.id].life - SkillEffect.Instance.target_life_old;
            ...
            if (SkillEffect.Instance.canStrikeBack) {
                ...
                cubeV2Pos = RectTransformUtility.WorldToScreenPoint (Camera.main, SkillEffect.Instance.target_roleIns.transform.position);
            }
            Debug.Log (cubeV2Pos);
    ...
    void GetChildCount (RectTransform managerValue, int length) {
        if (isAdd) {
            managerValue.GetChild (0).GetComponent<Image> ().sprite = ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.damageValuesShow[10] as Sprite;
            ShowValueImage (managerValue, length, ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.bloodValuesShows);
        } else if (isMaxDamage) {
            managerValue.GetChild (0).GetComponent<Image> ().sprite = ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.damageValuesShow[11] as Sprite;
            ShowValueImage (managerValue, length, ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.critValuesShow);
        } else {
            managerValue.GetChild (0).GetComponent<Image> ().sprite = ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.damageValuesShow[11] as Sprite;
            ShowValueImage (managerValue, length, ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.damageValuesShow);
    ...
    void SelectRoleSpineAni (string spineName, bool allAction = false, bool isLoop = false) {
        switch (spineName) {
            case "idle":
                RoleSpineAniManager.Instance.SendMsg (ButtonMsg.GetInstance.ChangeInfo ((ushort) MonsterSpineAniId.idle, target_roleIns.iddif, "normal", allAction, isLoop));
                break;
            case "hit":
                RoleSpineAniManager.Instance.SendMsg (ButtonMsg.GetInstance.ChangeInfo ((ushort) MonsterSpineAniId.attacked, target_roleIns.iddif, "hit", allAction, isLoop));
                break;
                // case "idle":
                // break;
                // case "idle":
    ...
    void SelectMonsterSpineAni (string spineName, bool allAction = false, bool isLoop = false) {
        switch (spineName) {
            case "idle":
                MonsterSpineAniManager.Instance.SendMsg (ButtonMsg.GetInstance.ChangeInfo ((ushort) RoleSpineAniId.idle, target_roleIns.id, "normal", allAction, isLoop));
                break;
            case "hit":
                MonsterSpineAniManager.Instance.SendMsg (ButtonMsg.GetInstance.ChangeInfo ((ushort) RoleSpineAniId.attacked, target_roleIns.id, "hit", allAction, isLoop));
                break;
                // case "idle":
                // break;
                // case "idle":
    ...
    void PassSkiiTargetSendAttackedFromExl (bool attackIsRole) {
        ...
        if (attackIsRole) {
            FixSpineAni.Instance.FixSpineAniFormFrame(usingSkill,target_roleIns,recordSkillName);
        } else {
            if (usingSkill.releaseobj == (int) TargetType.singleEnemy) {
                MonsterSpineAniManager.Instance.SendMsg (ButtonMsg.GetInstance.ChangeInfo ((ushort) RoleSpineAniId.attacked, target_roleIns.id, "hit", false, false));
            } else if (usingSkill.releaseobj == (int) TargetType.allEnemy) {
    ...
    public void ControlRoleSpineAni (string spineName, bool allAction = false, bool isLoop = false) {
        switch (spineName) {
            case "idle":
                RoleSpineAniManager.Instance.SendMsg (ButtonMsg.GetInstance.ChangeInfo ((ushort) RoleSpineAniId.idle, target_roleIns.id, "normal", allAction, isLoop));
                break;
            case "hit":
                RoleSpineAniManager.Instance.SendMsg (ButtonMsg.GetInstance.ChangeInfo ((ushort) RoleSpineAniId.attacked, target_roleIns.id, "hit", allAction, isLoop));
                break;
                // case "idle":
                // break;
                // case "idle":
    ...
    public void ControlMonsterSpineAni (string spineName, bool allAction = false, bool isLoop = false) {
        switch (spineName) {
            case "idle":
                MonsterSpineAniManager.Instance.SendMsg (ButtonMsg.GetInstance.ChangeInfo ((ushort) MonsterSpineAniId.idle, target_roleIns.iddif, "normal", allAction, isLoop));
                break;
            case "hit":
                MonsterSpineAniManager.Instance.SendMsg (ButtonMsg.GetInstance.ChangeInfo ((ushort) MonsterSpineAniId.attacked, target_roleIns.iddif, "hit", allAction, isLoop));
                break;
                // case "idle":
                // break;
                // case "idle":
