...
public class CreatSkeleton : UnitySingleton<CreatSkeleton>
{
    ...
    public Dictionary<int, SkeletonAnimation> skeletonsBattleAniDic = new Dictionary<int, SkeletonAnimation>();//战斗界面的人员
    public Dictionary<int, SkeletonAnimation> roleSkeletonsBattleAniDic = new Dictionary<int, SkeletonAnimation>();//战斗界面的人员 -- 用于技能伤害计算
    public List<SkeletonAnimation> skeletonsMonsterBattleAni = new List<SkeletonAnimation>();//战斗界面的NPC
    public List<SkeletonAnimation> monsterSkeletonsMonsterBattleAni = new List<SkeletonAnimation>();//战斗界面的NPC -- 用于技能伤害计算
    public Dictionary<int, SkeletonAnimation> skeletonsMonsterBattleAniDic = new Dictionary<int, SkeletonAnimation>();//战斗界面的NPC
    public Dictionary<int, SkeletonAnimation> monsterSkeletonsMonsterBattleAniDic = new Dictionary<int, SkeletonAnimation>();//战斗界面的NPC -- 用于技能伤害计算
    ...
    void BloodShow(Transform trans,RoleConfig role, int nameId, int id,List<GameObject> obj,bool isRole = false)
    {
        ...
        Text allBlood;
        ADDUIBattle.instance.bloodParent= GameObject.Find("CanvasBattle/PlayerPanel/bloodParent").GetComponent<RectTransform>();
        GameObject blood = GameObject.Instantiate(ResourcesLoadInfos.instance.bloodImage.gameObject, ADDUIBattle.instance.bloodParent) as GameObject;
    ...
    //怪物spine展示(通过配置表进行)
    public void CreatMonsterSkeletonAniInfos(Vector3 battlePlace, Transform trans, float x = -30f, float y = -140f, float z = 0f)
    {
        ...
        MonsterSpineAniManager.Instance.SendMsg(ButtonMsg.GetInstance.ChangeInfo((ushort)MonsterSpineAniId.idle,true));
    }
