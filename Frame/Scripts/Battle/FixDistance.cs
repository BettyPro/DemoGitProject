...
public class FixDistance : Singleton<FixDistance> {
    ...
    /// time
    /// </summary>
    public void FixFromTime (SkeletonAnimation attacker, Vector3 attackerOldPos, float needTime, float delayTime = 0.2f) {
    ...
    public void FixFromTimeOne (bool attackIsRole, SkeletonAnimation attacker, Vector3 attackerOldPos, float needTime, float delayTime = 0.2f) {
        MyTimerTool.Instance.wtime.AddTimeTask ((int tid) => {
            DOTween.To (() => attacker.transform.position, a => attacker.transform.position = a,
                attackerOldPos, needTime);
    ...
    /// frame
    /// </summary>
    public void FixFromFrame (SkeletonAnimation attacker, Vector3 attackerOldPos, float needTime, int delayFrame = 2) {
    ...
    public void FixFromFrameOne (string recordSkillName, RoleConfig roleconWhole, SkeletonAnimation attacker, Vector3 attackerOldPos, float needTime, int delayFrame = 2) {
        MyTimerTool.Instance.wtime.AddFrameTask ((int tid) => {
            DOTween.To (() => attacker.transform.position, a => attacker.transform.position = a,
                attackerOldPos, needTime);
