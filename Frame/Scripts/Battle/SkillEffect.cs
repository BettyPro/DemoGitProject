    ...
    public partial class SkillEffect : Singleton<SkillEffect>
    {
            ...
            if (ReadJsonfiles.Instance.skillDics.ContainsKey(int.Parse(obj.name)))
            {
                ...
                if (effectId != null && ReadJsonfiles.Instance.buffDics.ContainsKey(int.Parse(effectId)))
                {
                        ...
                        .skillDics[int.Parse(obj.name)].effectid1))].effectType.Equals("1"))
                    {
                        ...
                        Debug.LogError("技能value1：-- " + ReadJsonfiles.Instance.buffDics[(int.Parse(ReadJsonfiles
                                           .Instance
                                           .skillDics[int.Parse(obj.name)].effectid1))].value1);
                        Debug.LogError("技能value1：-- " + (float) ReadJsonfiles.Instance.buffDics[(int.Parse(ReadJsonfiles
                                           .Instance
                                           .skillDics[int.Parse(obj.name)].effectid1))].value1 / 100);
                    }
                }
        ...
        void JudgeAllEffectType(bool isRole = true)
        {
            ...
            else
            {
                ...
                {
                    target_roleIns = CreatSkeleton.Instance.monsterSkeletonsMonsterBattleAni[i]
        ...
        public void EffectType_One(RoleConfig roleconWhole)
        {
            ...
            skillDamage = BattleLogic.instance.isMaxDamage
                ? (float) int.Parse(roleconWhole.m_RoleInfo.critdamage.Split('%')[0]) / 100
                : 1;
        ...
        public void EffectType_Ten(RoleConfig roleconWhole)
        {
            ...
            if (attack_roleIns.iddif == 0)
            {
                RoleSpineAniManager.Instance.SendMsg(
                    ButtonMsg.GetInstance.ChangeInfo((ushort) MonsterSpineAniId.dizziness, target_roleIns.iddif, true));
            }
            else
            {
                MonsterSpineAniManager.Instance.SendMsg(
                    ButtonMsg.GetInstance.ChangeInfo((ushort) RoleSpineAniId.dizziness, target_roleIns.id, true));
            }
        ...
        public void CheckIsNeedChangeDizzinessState()
        {
            ...
            for (int i = 0; i < ADDUIBattle.instance.battleCurrentAll.Count; i++)
            {
                ...
                ske = ADDUIBattle.instance.battleCurrentAll[i].GetComponent<SkeletonAnimation>();
        ...
        {
            if (float.Parse(attack_roleIns.m_RoleInfo.life.ToString()) <
                ReadJsonfiles.Instance.buffDics[(int.Parse(effectId))].value1)
            {
        ...
        public float CheckIsThisSkillToAddAttack_EffectType_Eleven()
        {
            ...
            if (attack_roleIns.id == 20002 &&
                float.Parse(attack_roleIns.m_RoleInfo.life.ToString()) <
                ReadJsonfiles.Instance.buffDics[200020301].value1)
            {
        ...
        public void EffectType_Seventeen(RoleConfig roleconWhole)
        {
            ...
            SetConfig.TestVer("setceshi", singleTestBool: true, changBool: isAgainAttack, debugInfo: "再动一回合开启");
        }
        ...
        public float ProjectTeammate_EffectType_Nineteen(float damage)
        {
            ...
            {
                RoleConfig role10004 =
                    CreatSkeleton.Instance.roleSkeletonsBattleAniDic[10004].GetComponent<RoleConfig>();
        ...
        {
            if (target_roleIns.m_RoleInfo.life - BattleLogic.instance.damage * skillDamage * skillCoefficient <
                ReadJsonfiles.Instance.buffDics[(int.Parse(effectId))].value1)
            {
                isDizziness = true;
                checkBools[2] = isDizziness;
            ...
            if (CreatSkeleton.Instance.BloodsDic.ContainsKey(id))
            {
                ...
                if (CheckIsChildHaveOwn)
                {
                    ...
                    if (ADDUIBattle.instance.ResourcesLoadBattleSceneInfos.roleBuffImagesDics.ContainsKey(str))
                    {
                        ...
                        CreatContinueBoutText(obj.transform,
                            ReadJsonfiles.Instance.buffDics[(int.Parse(effectId))].continueBout, str, id);
                    }

                    obj.GetComponent<RectTransform>().position =
                        target_roleIns.transform.position + new Vector3(0, 20, 0);
                    // //TOTO 找到刚添加的新buff
                    // FindCurrentNewBuff(str);
                    //TOTO 计算buff回合
                    CountBuffTime();
        ...
        void RecountBuffTime(string buffName)
        {
            ...
            for (int i = 0; i < currentTarget.Length; i++)
            {
                ...
                {
                    currentTarget[i].GetChild(0).GetComponent<Text>().text = ReadJsonfiles.Instance
                        .buffDics[(int.Parse(effectId))].continueBout.ToString();
                }
            }
        }
