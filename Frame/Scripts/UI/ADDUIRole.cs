...
public class ADDUIRole : UIBase
{
    ...
    public void CreatRoleSingleInfo(Transform trans,float x= 10f, float y = 195f, float z = 0f, bool fixPos = false, float width = 150f,float height = 110f,bool isRole = true,int length = 5)
    {
        for (int i = 0; i < 5; i++)
        {
            ...
            if (isRole)
            {
                ...
                obj.AddComponent<UIBehaviour>().AddButtonListener<UIBehaviour>(RoleShowSingle, obj.GetComponent<UIBehaviour>(), roleUse);
            }
            else
            {
                speedImages.Add(obj);
